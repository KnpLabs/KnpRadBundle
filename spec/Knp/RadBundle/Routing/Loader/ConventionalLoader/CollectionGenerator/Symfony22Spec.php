...
namespace spec\Knp\RadBundle\Routing\Loader\ConventionalLoader\CollectionGenerator;

use PhpSpec\ObjectBehavior;

use InvalidArgumentException;

class Symfony22Spec extends ObjectBehavior
{
    function let()
    {
    ...
    function it_should_load_simple_collection_by_conventions()
    {
        $routes = $this->generate('App:Cheeses', null);
    ...
    function it_should_load_collections_with_custom_prefix()
    {
        $routes = $this->generate('App:Cheeses','/custom/prefix');
    ...
    function it_should_load_collections_with_specified_actions()
    {
        $routes = $this->generate('App:Cheeses', array(
    ...
    function it_should_load_collections_with_specified_action_patterns()
    {
        $routes = $this->generate('App:Cheeses', array(
            'prefix'    => '/custom/prefix',
            'resources' => array(
    ...
    function it_should_load_collections_with_custom_parameters()
    {
        $routes = $this->generate('App:Cheeses', array(
            'prefix'    => '/custom/prefix',
            'resources' => array(
    ...
    function it_should_merge_defaults_and_requirements_into_child_routes()
    {
        $routes = $this->generate('App:Cheeses', array(
            'prefix'       => '/custom/prefix',
    ...
    function it_should_load_simple_routes_with_pattern()
    {
        $routes = $this->generateRoute('App:Cheeses:list', '/cheeses/{id}/list');
    ...
    function it_should_load_simple_routes_with_params()
    {
        $routes = $this->generateRoute('App:Cheeses:list', array(
    ...
    function it_should_support_options()
    {
        $routes = $this->generateRoute('App:Cheeses:list', array(
            'pattern' => '/',
            'options' => array('test' => 'foo'),
        ));

        $list = $routes->get('app_cheeses_list');
        $list->getOptions()->shouldReturn(array(
            'compiler_class' => 'Symfony\Component\Routing\RouteCompiler',
            'test' => 'foo',
        ));
    }

    function it_should_create_proper_route_for_namespaced_controller()
    {
        $routes = $this->generateRoute('App:Admin\Cheeses:list', '/admin/cheeses/list-cheeses');
    ...
    function it_should_generate_proper_prefix_for_namespaced_controller()
    {
        $routes = $this->generate('App:Admin\Cheeses', null);
    ...
    function it_should_properly_transform_camel_cased_classes_and_groups()
    {
        $routes = $this->generate('SuperAppBundle:AdminControllers\Cheeses', null);

        $routes->get('superAppBundle_adminControllers_cheeses_index')->shouldNotReturn(null);
    }

    function it_should_throw_exception_if_unsupported_controller_route_param_provided()
    {
        $this->shouldThrow(new InvalidArgumentException(
            '`unsupported_key` parameter is not supported by `App:Admin\Cheeses` controller route. Use one of [prefix, defaults, requirements, options, collections, resources].'
        ))->duringGenerate('App:Admin\Cheeses', array(
            'unsupported_key' => true
        ));
    }

    function it_should_throw_exception_if_unsupported_action_route_param_provided()
    {
        $this->shouldThrow(new InvalidArgumentException(
            '`unsupported_key` parameter is not supported by `App:Admin\Cheeses:show` action route. Use one of [pattern, defaults, requirements, options].'
        ))->duringGenerateRoute('App:Admin\Cheeses:show', array(
            'unsupported_key' => true
        ));
    }

    function it_should_throw_exception_if_user_uses_pattern_key_instead_of_prefix()
    {
        $this->shouldThrow(new InvalidArgumentException(
            'The `pattern` is only supported for actions, if you want to prefix all the routes of the controller, use `prefix` instead.'
        ))->duringGenerate('App:Cheeses', array(
